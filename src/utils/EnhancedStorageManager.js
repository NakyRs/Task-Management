...
class EnhancedStorageManager {
  ...
  importData(importData) {
    ...
    try {
      ...
      if (importData.appName !== this.appName) {
        console.warn(
          `Importing data from different app: ${importData.appName}`
        );
      }
  ...
  getStorageInfo() {
    ...
    try {
      ...
      return {
        ...
        usagePercentage:
          totalSize > 0 ? ((appSize / totalSize) * 100).toFixed(2) : 0,
      };
    } catch (error) {
  ...
  _updateMetadata(entity, timestamp) {
    // skip metadata update entitiy '_metadata' agar tidak infinite loop
    if (entity === '_metadata') {
      return;
    }
    ...
    //simpan metdata langsung tanpa trigger _updateMetadata lagi
